...
export default function RecruitmentDetails() {
    ...
    const { loading: applicantsLoading, data: applicantsData, refetch: refetchApplicants } = useQuery(GET_RECRUITMENT_APPLICANTS, {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if (!applicantsLoading && applicantsData && applicantsData.recruitmentApplicants) {
          setApplicants(applicantsData.recruitmentApplicants);
        } else {
          refetchApplicants();
          if (applicantsData) {
            setApplicants(applicantsData.recruitmentApplicants);
    ...
    return (
        <div className='flex flex-col w-screen gap-4 mt-4'>
            <div className='flex flex-row gap-3 justify-start pl-2 items-center'>
                <div className='flex flex-row gap-2 items-center'>
                    <img src={user.profilePicture} alt="" className='w-32 h-32 shadow-lg' style={{borderRadius: "50%"}} />
                    <h3 className='uppercase text-base font-open-sans text-darkBg drak:text-white'>{user.firstname} {user.lastname}</h3>
                </div>
            </div>
                ...
                <table className='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 rounded-b-md'>
                    ...
                    <tbody className=''>
                        {applicants.map((applicant) => (
                            <tr key={applicant.id} className="hover:bg-gray-100 dark:hover:bg-gray-500 cursor-pointer border-b  dark:border-gray-700">
